...
func NewManager(cfg config.RateLimitingConfig) *Manager {
	m := &Manager{
	...
	for _, rule := range cfg.Rules {
		...
		if _, exists := m.limiters[rule.Name]; exists {
			log.Fatalf("[限流管理器] 致命错误: 发现重复的限流规则名称 '%s'", rule.Name)
		}
		...
		case "memory_token_bucket":
			...
			if settings.Capacity <= 0 || settings.RefillRate <= 0 {
				log.Fatalf("[限流管理器] 致命错误: 规则 '%s' 的 capacity 和 refillRate 必须为正数", rule.Name)
			}
		...
		if err != nil {
			log.Fatalf("[限流管理器] 致命错误: 创建规则 '%s' 失败: %v", rule.Name, err)
		}

		m.limiters[rule.Name] = newLimiter
		log.Printf("[限流管理器] 成功加载规则 '%s' (类型: %s)", rule.Name, rule.Type)
	}
